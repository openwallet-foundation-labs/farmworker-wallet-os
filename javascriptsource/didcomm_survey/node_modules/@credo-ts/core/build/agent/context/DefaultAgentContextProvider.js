...
let DefaultAgentContextProvider = class DefaultAgentContextProvider {
    ...
    async getAgentContextForContextCorrelationId(contextCorrelationId) {
        if (contextCorrelationId !== this.agentContext.contextCorrelationId) {
            throw new error_1.CredoError(`Could not get agent context for contextCorrelationId '${contextCorrelationId}'. Only contextCorrelationId '${this.agentContext.contextCorrelationId}' is supported.`);
        }
    ...
    async endSessionForAgentContext(agentContext) {
        ...
        if (agentContext.contextCorrelationId !== this.agentContext.contextCorrelationId) {
            throw new error_1.CredoError(`Could not end session for agent context with contextCorrelationId '${agentContext.contextCorrelationId}'. Only contextCorrelationId '${this.agentContext.contextCorrelationId}' is provided by this provider.`);
        }
