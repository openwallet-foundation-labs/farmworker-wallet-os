...
exports.getConnectionlessOutboundMessageContext = exports.getOutboundMessageContext = void 0;
 ...
 */
async function getOutboundMessageContext(agentContext, { message, connectionRecord, associatedRecord, lastReceivedMessage, lastSentMessage, }) {
    ...
    if (!lastReceivedMessage) {
        throw new error_1.CredoError('No connection record and no lastReceivedMessage was supplied. For connection-less exchanges the lastReceivedMessage is required.');
    }
    if (!associatedRecord) {
        throw new error_1.CredoError('No associated record was supplied. This is required for connection-less exchanges to store the associated ~service decorator on the message.');
    }
    // Connectionless
...
async function getConnectionlessOutboundMessageContext(agentContext, { message, lastReceivedMessage, lastSentMessage, associatedRecord, }) {
    ...
    if (!ourService) {
        throw new error_1.CredoError(`Could not determine our service for connection-less exchange for message ${message.id}.`);
    }
    if (!recipientService) {
        throw new error_1.CredoError(`Could not determine recipient service for connection-less exchange for message ${message.id}.`);
    }
...
async function getServicesForMessage(agentContext, { lastSentMessage, lastReceivedMessage, message, outOfBandRecord, }) {
    var _a, _b;
    let ourService = (_a = lastSentMessage === null || lastSentMessage === void 0 ? void 0 : lastSentMessage.service) === null || _a === void 0 ? void 0 : _a.resolvedDidCommService;
    let recipientService = (_b = lastReceivedMessage.service) === null || _b === void 0 ? void 0 : _b.resolvedDidCommService;
    ...
    if ((outOfBandRecord === null || outOfBandRecord === void 0 ? void 0 : outOfBandRecord.role) === oob_1.OutOfBandRole.Sender) {
        ...
        if (!ourService) {
            ourService = await outOfBandService.getResolvedServiceForOutOfBandServices(agentContext, outOfBandRecord.outOfBandInvitation.getServices());
        }
        if (!recipientService) {
            throw new error_1.CredoError(`Could not find a service to send the message to. Please make sure the connection has a service or provide a service to send the message to.`);
        }
        // We have created the oob record with a message, that message should be provided here as well
        if (!lastSentMessage) {
            throw new error_1.CredoError('Must have lastSentMessage when out of band record has role Sender');
        }
    }
        ...
        if (!recipientService) {
            recipientService = await outOfBandService.getResolvedServiceForOutOfBandServices(agentContext, outOfBandRecord.outOfBandInvitation.getServices());
        }
    ...
    // we check in separate if statements to provide a more specific error message
        ...
        if (lastSentMessage && !ourService) {
            ...
            throw new error_1.CredoError(`No out of band record associated and missing our service for connection-less exchange for message ${message.id}, while previous message has already been sent.`);
        }
        if (!recipientService) {
            ...
            throw new error_1.CredoError(`No out of band record associated and missing recipient service for connection-less exchange for message ${message.id}.`);
        }
    }
...
//# sourceMappingURL=getOutboundMessageContext.js.map