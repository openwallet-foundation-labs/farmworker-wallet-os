...
class MediationRecord extends BaseRecord_1.BaseRecord {
    ...
    assertReady() {
        if (!this.isReady) {
            throw new error_1.CredoError(`Mediation record is not ready to be used. Expected ${MediationState_1.MediationState.Granted}, found invalid state ${this.state}`);
        }
    }
    assertState(expectedStates) {
        ...
        if (!expectedStates.includes(this.state)) {
            throw new error_1.CredoError(`Mediation record is in invalid state ${this.state}. Valid states are: ${expectedStates.join(', ')}.`);
        }
    }
    assertRole(expectedRole) {
        if (this.role !== expectedRole) {
            throw new error_1.CredoError(`Mediation record has invalid role ${this.role}. Expected role ${expectedRole}.`);
        }
    }
}
