...
let GenericRecordService = class GenericRecordService {
    ...
    async save(agentContext, { content, tags, id }) {
        ...
        try {
            ...
            throw new error_1.CredoError(`Unable to store the genericRecord record with id ${genericRecord.id}. Message: ${error}`);
        }
    }
    async delete(agentContext, record) {
        try {
            ...
            throw new error_1.CredoError(`Unable to delete the genericRecord record with id ${record.id}. Message: ${error}`);
        }
    }
    ...
    async update(agentContext, record) {
        try {
            ...
            throw new error_1.CredoError(`Unable to update the genericRecord record with id ${record.id}. Message: ${error}`);
        }
    }
