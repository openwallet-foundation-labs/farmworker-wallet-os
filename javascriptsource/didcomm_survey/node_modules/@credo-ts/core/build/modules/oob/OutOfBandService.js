...
let OutOfBandService = class OutOfBandService {
    ...
    async createFromImplicitInvitation(agentContext, config) {
        ...
        if (!createdDid) {
            throw new error_1.CredoError(`Referenced public did ${did} not found.`);
        }
    ...
    async processHandshakeReuseAccepted(messageContext) {
        ...
        if (!outOfBandRecord) {
            throw new error_1.CredoError('No out of band record found for handshake-reuse-accepted message');
        }
        // Assert
    ...
    async getResolvedServiceForOutOfBandServices(agentContext, services) {
        ...
        throw new error_1.CredoError('Could not extract a service from the out of band invitation.');
    }
};
