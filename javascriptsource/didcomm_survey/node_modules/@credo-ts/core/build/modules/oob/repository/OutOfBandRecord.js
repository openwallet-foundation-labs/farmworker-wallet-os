...
class OutOfBandRecord extends BaseRecord_1.BaseRecord {
    ...
    getTags() {
        var _a;
        return Object.assign(Object.assign({}, this._tags), { role: this.role, state: this.state, invitationId: this.outOfBandInvitation.id, threadId: this.outOfBandInvitation.threadId, invitationRequestsThreadIds: (_a = this.outOfBandInvitation
                .getRequests()) === null || _a === void 0 ? void 0 : _a.map((r) => (0, thread_1.getThreadIdFromPlainTextMessage)(r)) });
    }
    assertRole(expectedRole) {
        if (this.role !== expectedRole) {
            throw new error_1.CredoError(`Invalid out-of-band record role ${this.role}, expected is ${expectedRole}.`);
        }
    }
    assertState(expectedStates) {
        ...
        if (!expectedStates.includes(this.state)) {
            throw new error_1.CredoError(`Invalid out-of-band record state ${this.state}, valid states are: ${expectedStates.join(', ')}.`);
        }
    }
}
