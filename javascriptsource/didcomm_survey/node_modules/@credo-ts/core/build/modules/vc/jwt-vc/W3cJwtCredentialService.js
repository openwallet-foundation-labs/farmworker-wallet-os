...
let W3cJwtCredentialService = class W3cJwtCredentialService {
    ...
    async signCredential(agentContext, options) {
        ...
        if (!(0, utils_1.isDid)(options.verificationMethod)) {
            throw new error_1.CredoError(`Only did identifiers are supported as verification method`);
        }
        ...
        try {
            ...
            try {
                ...
                if (!signatureResult.isValid) {
                    validationResults.validations.signature = {
                        isValid: false,
                        error: new error_1.CredoError('Invalid JWS signature'),
                    };
                }
                else {
                    validationResults.validations.signature = {
            ...
            if (credential.issuerId !== issuerVerificationMethod.controller) {
                validationResults.validations.issuerIsSigner = {
                    isValid: false,
                    error: new error_1.CredoError(`Credential is signed using verification method ${issuerVerificationMethod.id}, while the issuer of the credential is '${credential.issuerId}'`),
                };
            }
            else {
                validationResults.validations.issuerIsSigner = {
            ...
            if (!issuerIsSigner) {
                validationResults.validations.issuerIsSigner = {
                    isValid: false,
                    error: new error_1.CredoError('Credential is not signed by the issuer of the credential'),
                };
            }
            else {
                validationResults.validations.issuerIsSigner = {
            ...
            if (verifyCredentialStatus && !credential.credentialStatus) {
                ...
                validationResults.validations.credentialStatus = {
                    isValid: false,
                    error: new error_1.CredoError('Verifying credential status is not supported for JWT VCs'),
                };
            }
        ...
        try {
            ...
            try {
                ...
                if (options.challenge !== presentation.jwt.payload.additionalClaims.nonce) {
                    throw new error_1.CredoError(`JWT payload 'nonce' does not match challenge '${options.challenge}'`);
                }
                const audArray = (0, utils_1.asArray)(presentation.jwt.payload.aud);
                if (options.domain && !audArray.includes(options.domain)) {
                    throw new error_1.CredoError(`JWT payload 'aud' does not include domain '${options.domain}'`);
                }
                validationResults.validations.dataModel = {
                    isValid: true,
                };
            ...
            try {
                ...
                if (!signatureResult.isValid) {
                    validationResults.validations.presentationSignature = {
                        isValid: false,
                        error: new error_1.CredoError('Invalid JWS signature on presentation'),
                    };
                }
                else {
                    validationResults.validations.presentationSignature = {
            ...
            if (presentation.holderId && proverVerificationMethod.controller !== presentation.holderId) {
                validationResults.validations.holderIsSigner = {
                    isValid: false,
                    error: new error_1.CredoError(`Presentation is signed using verification method ${proverVerificationMethod.id}, while the holder of the presentation is '${presentation.holderId}'`),
                };
            }
            else {
            ...
            validationResults.validations.credentials = await Promise.all(credentials.map(async (credential) => {
                ...
                if (credentialSubjectIds.length > 0 && !presentationAuthenticatesCredentialSubject) {
                    credentialSubjectAuthentication = {
                        isValid: false,
                        error: new error_1.CredoError('Credential has one or more credentialSubject ids, but presentation does not authenticate credential subject'),
                    };
                }
                else {
                    credentialSubjectAuthentication = {
    ...
    async getVerificationMethodForJwtCredential(agentContext, options) {
        ...
        // this is a full did url (todo check if it contains a #)
            ...
            if (signerId && didDocument.id !== signerId) {
                throw new error_1.CredoError(`kid '${kid}' does not match id of signer (holder/issuer) '${signerId}'`);
            }
        }
        else {
            if (!signerId) {
                throw new error_1.CredoError(`JWT 'iss' MUST be present in payload when no 'kid' is specified`);
            }
            ...
            if (verificationMethods.length === 0) {
                ...
                throw new error_1.CredoError(`Multiple verification methods found for signer '${signerId}' and key type '${jwkClass.keyType}' for alg '${credential.jwt.header.alg}'. Unable to determine which public key is associated with the credential.`);
            }
        ...
        if (signerId && verificationMethod.controller !== signerId) {
            throw new error_1.CredoError(`Verification method controller '${verificationMethod.controller}' does not match the signer '${signerId}'`);
        }
