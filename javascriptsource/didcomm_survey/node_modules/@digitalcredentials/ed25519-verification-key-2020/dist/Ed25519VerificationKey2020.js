...
class Ed25519VerificationKey2020 extends cryptoLd.LDKeyPair {
  /**
  ...
  static fromEd25519VerificationKey2018({keyPair} = {}) {
    const publicKeyMultibase = _encodeMbKey(
      MULTICODEC_ED25519_PUB_HEADER, base58btc__namespace.decode(keyPair.publicKeyBase58));
    ...
    if(keyPair.privateKeyBase58) {
      keyPair2020.privateKeyMultibase = _encodeMbKey(
        ...
        base58btc__namespace.decode(keyPair.privateKeyBase58));
    }
  ...
  static fromJsonWebKey2020({id, type, controller, publicKeyJwk} = {}) {
    ...
    const publicKeyMultibase = _encodeMbKey(
      ...
      base64url__namespace.decode(publicKeyBase64Url));
  ...
  get _publicKeyBuffer() {
    ...
    const publicKeyMulticodec =
      base58btc__namespace.decode(this.publicKeyMultibase.substr(1));
  ...
  get _privateKeyBuffer() {
    ...
    const privateKeyMulticodec =
      base58btc__namespace.decode(this.privateKeyMultibase.substr(1));
    // remove multicodec header
  ...
  toJwk({publicKey = true, privateKey = false} = {}) {
    ...
    if(publicKey) {
      jwk.x = base64url__namespace.encode(this._publicKeyBuffer);
    }
    if(privateKey) {
      jwk.d = base64url__namespace.encode(this._privateKeyBuffer);
    }
    return jwk;
  }
   ...
   */
  async jwkThumbprint() {
    const publicKey = base64url__namespace.encode(this._publicKeyBuffer);
    const serialized = `{"crv":"Ed25519","kty":"OKP","x":"${publicKey}"}`;
    const data = new TextEncoder().encode(serialized);
    return base64url__namespace.encode(
  ...
  verifyFingerprint({fingerprint} = {}) {
    ...
    try {
      fingerprintBuffer = base58btc__namespace.decode(fingerprint.substr(1));
...
function _encodeMbKey(header, key) {
  ...
  return MULTIBASE_BASE58BTC_HEADER + base58btc__namespace.encode(mbKey);
}

exports.Ed25519VerificationKey2020 = Ed25519VerificationKey2020;
