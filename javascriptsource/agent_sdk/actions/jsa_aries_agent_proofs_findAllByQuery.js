 ...
 */
export async function jsa_aries_agent_proofs_findAllByQuery(agent_key, query) {
	// BEGIN USER CODE
	try{
		if(agent_key==null)/*mandatory*/return Promise.reject("Invalid agent_key parameter");
		if(query==null)/*mandatory*/return Promise.reject("Invalid query parameter");
		try{
		...
		return Promise.resolve(JSON.stringify(await agent.proofs.findAllByQuery(query)));
	}catch(e){
