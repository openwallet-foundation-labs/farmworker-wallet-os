 ...
 */
export async function jsa_aries_agent_modules_userProfile_getUserProfileData(agent_key) {
	// BEGIN USER CODE
	try{
		if(agent_key==null)return Promise.reject("Invalid agent_key parameter");
		let agent=support.cache.get(agent_key);
		if(agent==null)return Promise.reject("Agent not found in cache");
		return(Promise.resolve(JSON.stringify(await agent.modules.userProfile.getUserProfileData(),0,2)));
	}catch(e){
